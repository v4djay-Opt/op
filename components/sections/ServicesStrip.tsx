...
export const ServicesStrip = memo(function ServicesStrip() {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    (async () => {
      ...
      ctx = gsap.context(() => {
        // Sync GSAP state with the CSS initial state (non-first cards below)
        gsap.set(cards.slice(1), { y: "100%" });

        // ── pinned timeline ──────────────────────────────────────────
        // Section stays fixed while all (N-1) card reveals play out.
        ...
        // Build one step per card reveal
        for (let i = 1; i < cards.length; i++) {
          const pos = i - 1; // timeline label / position

          // Slide the incoming card up from below
          tl.to(cards[i], { y: 0, ease: "power2.out", duration: 1 }, pos);

          // Push every previously visible card further back in the stack
          for (let j = 0; j < i; j++) {
            const depth = i - j;               // 1 = just behind, 2 = two deep, …
            const scale = 1 - depth * 0.04;    // 0.96 → 0.92 → 0.88 …
  ...
  return (
    <section className="relative pt-20 lg:pt-28 pb-20 px-4">
      <div className="mx-auto max-w-5xl">
        ...
        {/*
          .services-stack-grid  → CSS grid (desktop) or flex-col (mobile) — see globals.css
